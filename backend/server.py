...
def get_expenses(expense_date : date):
    expenses_1 = db_helper.fetch_expenses_for_date(expense_date)
    if expenses_1 is None:
        raise HTTPException(status_code=500, detail="Failed to retrieve expense from the database.")

    return expenses_1

@app.post("/expenses/{expense_date}")
def add_or_update_expenses_for_date(expense_date : date, expenses: List[Expense]):
